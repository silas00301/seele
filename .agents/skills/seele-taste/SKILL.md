...
- Build a square now-playing tile like macOS: small artwork at the upper left, track text below it, and flat transport controls along the bottom. Put a menu bar player's artwork before its label. Let both open one larger media panel with artwork and transport controls. Show a clickable, draggable timeline only when the player reports seek, position, and duration support.
