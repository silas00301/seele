...
- Give AirPods and over-ear headphones distinct silhouettes. Show the AirPods label when AirPods are connected, with priority over other supported headphones; otherwise use Headphones. Let Audio mirror several selected outputs through an explicit Multiple outputs mode, while microphone selection stays exclusive.
