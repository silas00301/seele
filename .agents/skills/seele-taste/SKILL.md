...
- Keep shell integrations optional when they need account setup. Reuse an existing CLI login or a private local connection file, show stale/error state explicitly, and require an explicit action before changing a connected device or opening a remote item. Home Assistant keeps its house icon visible before configuration and offers setup in the panel; save its token through the standard Secret Service interface, with only display preferences and the URL in its private file. Keep selected Home Assistant sensors as named readouts in room cards or Favorites, and round numeric readings to one decimal place, omitting a trailing `.0`.
...
- Make an everyday destructive action recoverable rather than merely confirmed, and reach that by making the safe path shorter than the unsafe one instead of shadowing the unsafe one. An interactive alias covers neither scripts, nor non-interactive ssh, nor `run0`, so a shadowed `rm` teaches a habit that fails exactly where the stakes are highest; leave the original command's semantics intact and give the recoverable one the shorter name. Bound what recoverability costs with a persistent user timer instead of asking anyone to empty anything, and leave the one genuinely irreversible command in the set without a shortcut.
...
- Open a file in a native, keyboard-driven viewer rather than in the browser or in nothing at all: images in imv, video and audio in mpv, PDFs and EPUBs in zathura, text in the configured Neovim, and a directory in Yazi, with the terminal applications opened by the configured terminal. Keep one owner for those defaults, leaving the browser the web documents and URL schemes it actually handles, and let a glance float: an image opened from a file manager or a notification interrupts a layout and should not rearrange it.
- Support the mouse where it is convenient, but make the complete primary workflow keyboard-accessible.
- Hide passive chrome intelligently: auto-hide docks, compact launchers, suppress empty media/privacy indicators, and expose rich previews or status only when relevant. Keep the network panel's IP addresses behind a collapsed, scroll-bounded disclosure rather than printing every interface address on opening.
...
- Treat an associated local notification image as sender identity rather than body media. Let it lead the card in the rounded icon slot, badge the sending application's icon at the lower-right corner, and fall back to that application icon while the identity image is unavailable.
- Give silence a length as well as a switch. Alongside indefinite Do Not Disturb, offer a small set of quiet periods — 15 minutes, an hour, four hours — as one well with the running period lit, so the control that started it is also what reports it, and put the hour it ends on the group's rule. Hold an absolute deadline rather than a countdown, so suspending the machine does not extend the quiet; end it without replaying what it suppressed; and let throwing the switch by hand cancel a running period rather than stacking with it.
...
- Draw a value that runs along a range as a level, not as a number between two nudge buttons: the player's own volume is a track the pointer can land anywhere in, with the mark that reports how loud it is doubling as the control that silences it, and a silenced player comes back to the level it was taken from. Offer a set of values a control can hold — playback speeds — as one well with the running one lit, so what else is on offer is visible without clicking through it, and keep the keyboard stepping through the set as one control.
...
- Theme the mouse pointer from those same shared Catppuccin values rather than a literal variant name, and declare its size once so the compositor, the toolkits, XWayland, the greeter, and the lock screen all draw the same pointer at the same size. A pre-session surface needs the theme in the system profile, since it cannot read the user's icons.
...
- Use one smart screenshot picker for click-to-window or monitor selection and drag-to-region capture. Freeze the displayed frame until capture, reserve predictable timestamped filenames safely, copy the final image only after optional annotation, and expose annotation as a modifier variant of the same picker. An upload variant must keep the local image, ask for consent in a native warning dialog that says the destination is public and third-party, use a hard-to-guess link with a short expiry, and fall back to copying the image on decline or failure.
...
- Use **Super + Space** for a centered, keyboard-first quick AI prompt on the focused output. Map its existing-theme shell surface immediately, but start the model only on Send. Make every optional source an explicit `@mention`; resolve explicit mentions together on Send, with no earlier reads or separate approval steps. Model-requested clipboard/selection context still needs one-time approval, and model-requested screen context still needs Capture and preview. Keep prompt context, answers, captures, and the short-lived follow-up session private to the open panel. Enter sends or copies; Ctrl + Enter restores the exact original window and inserts without moving the pointer.
- Mount removable disks as soon as they are plugged in, carry the drivers foreign filesystems need, and let the notification be the whole feedback. Do not add a second tray for something Seele Shell's notification server already announced; give the per-device menu away and use **Super + Shift + E** instead, which unmounts and powers off every attached device at once. Send a browse action to Yazi in Ghostty rather than an unspecified desktop opener, and notify on eject only when a device is still in use.
- Keep a scratchpad terminal one key away on the key left of the 1, the same place macOS summons Ghostty's quick terminal from, and let the compositor own that gesture while the pinned Ghostty cannot be toggled from outside. Hide it on a special workspace rather than a numbered one, spawn it from the workspace becoming empty so nothing tracks whether it runs, give it a tmux session that outlives its window, and center it like the quick AI prompt instead of anchoring it to the edge the bar owns.
- Prefer **Maple Mono NF CN** for terminal and monospace UI; Geist Mono is an available secondary font.
- Correct the screen's colour after dark the way every other machine here does, on clock hours rather than on sunrise and sunset, since those need coordinates that do not belong in this repository. Step into it instead of switching once, ask the compositor for the transform so captures and screen shares keep true colours, and keep one key that takes the tint out of the way immediately for the work that cannot have it.
...
- Resolve names through an authenticated encrypted channel to a provider whose business is not advertising or content delivery; Quad9 is the established choice. Prefer strict DNS-over-TLS with the server name pinned over an opportunistic mode the network can downgrade, and let the upstream resolver own DNSSEC rather than adding a second validator that fails closed. Keep the host silent on the LAN: resolve multicast names without registering one, and leave LLMNR off.
...
- Keep GitHub notification triage automatic through the shared broker while showing every API notification, including pending/failed analysis. Only the two urgent priorities create desktop alerts, and their action opens the shell detail. Done is explicit; defer unavailable public-API operations to GitHub instead of claiming local state has synchronized.
- Manage GitHub CLI authentication through `gh auth login`; keep its token in gh's local state rather than injecting it through a 1Password shell plugin.
