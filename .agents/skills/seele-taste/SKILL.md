...
- Make a notification that carries an action clickable wherever it is current. Let every popup retire to the notification panel after ten seconds, even when it carries an action, and give the popup a visible close button for doing that immediately. A history record is no longer actionable.
...
- Keep scroll indicators thin and show them only when content exceeds the viewport and the pointer is over the popup, including in views where scrolling is normally expected. Keep a thumb visible until an active drag ends.
