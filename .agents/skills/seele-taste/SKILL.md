...
- Use **Super + Space** for a centered, keyboard-first quick AI prompt on the focused output. Map its existing-theme shell surface immediately, but start the model only on Send. Make every optional source an explicit `@mention`; require visible one-time permission before reading clipboard or primary-selection text, and show the exact captured output before `@screen` can be sent. Keep prompt context, answers, captures, and the short-lived follow-up session private to the open panel. Enter sends or copies; Ctrl + Enter restores the exact original window and inserts without moving the pointer.
...
| Notes and voice memos | Standalone Seele Notes, with local storage and the identical shared shell style |
