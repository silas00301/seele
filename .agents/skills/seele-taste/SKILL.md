...
- Keep remote shell access on Tailscale. Expose Tailscale SSH as a persistent Control Center toggle for tailnet-policy access; if OpenSSH remains as a fallback, expose it only on `tailscale0` and require public keys.
