...
- Hold a surface's hover state for as long as the pointer is anywhere on that surface, including while it rests on a control the surface carries or on one sitting just outside the area that reports it. A card that goes cold under its own dismiss button, or a row that drops as the pointer reaches the button its hover just revealed, reads as a flicker and as a hitbox that does not match the shape being pointed at.
