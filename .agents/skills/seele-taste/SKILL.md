...
- Make a notification that carries an action clickable wherever it is shown, and let an actionable notification wait for the user rather than expiring on the ordinary timeout. A record of something already dismissed is a record, not a button.
...
- Arrange an aggregate control panel on a four-column grid when modules have different weights. Keep related connectivity rows compact inside one two-column card, put a square now-playing tile at the upper right, and use short device tiles below it. Split that short row when a conditional device appears instead of making every card taller.
- Group output and microphone sliders in one sound card. Keep the slider and mute actions direct, and use the card header to open the full Audio panel or drag the one sound module.
- Build a square now-playing tile like macOS: small artwork at the upper left, track text below it, and flat transport controls along the bottom. Put a menu bar player's label before its artwork. Let both open one larger media panel with artwork and transport controls. Show a clickable, draggable timeline only when the player reports seek, position, and duration support.
...
- Use 24-hour time, ISO-style dates, and Europe/Berlin when a timezone must be chosen. Show seconds in an expanded clock while keeping the menu bar clock compact. The desktop language is primarily English; preserve the German keyboard layout where host input is concerned.
