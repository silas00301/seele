...
- Let output gain reach 150% while keeping microphone gain capped at 100%. Scale each slider and its OSD to its real range, mark the output's 100% point, and keep wheel and keyboard steps subject to the same limits.
...
- Decide a size, weight, tracking, space, control height, elevation, or duration once, in that block, and name the step for the role it plays rather than for its value. A new control picks a step; it does not introduce a value. A glyph normally takes the step above the text beside it, and a meter or a rounded end is rounded on its own height rather than on a literal that outgrows the bar it is drawn in.
- Carry hierarchy in weight instead of bolding every label: DemiBold for a title or an active label, Medium for a quiet uppercase rule, Light for the large numerals a card leads with, and nothing set to plain bold. Track the uppercase section rules; leave everything else untracked, and keep those rules to one size and one colour wherever they appear.
- Build depth out of translucency rather than out of greys. A card is a tint of the panel's own material, a list row a lighter tint again, a track or well is cut back toward the base, and one opaque step is reserved for a control that overlaps a row whose own fill moves under the pointer. Lift a card with a hairline rather than with a heavier fill.
