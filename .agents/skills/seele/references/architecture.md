...
and use the root Cargo workspace, lock and release policy. `runtime` owns bounded
process and Unix-socket I/O, cancellation, private atomic writes, timestamp
formatting, redaction and broker client lifecycle. `broker`, `maintenance`,
`prompt`, `integrations`, `failure-analysis`, `shell-ai`, `config-tools`,
`desktop-tools`, `repo-tools` and the existing `tools` crate reuse it.

`packages/core/native.nix` in the shell is the sole native derivation builder.
Parent leaves expose Linux services through the shell flake and portable helpers
through `inputs.seele-shell.lib.mkNativePackage`. Native feature wrappers supply
required executable dependencies; isolated fixtures target unwrapped binaries.
`repo-tools` owns `check`, package-pin updates, submodule synchronization and the
Jujutsu flip/PR helpers and the privileged generation activation helper. `desktop-tools` owns screenshots, Spotify status, stopped-Brave Qt preferences,
and the existing Windows reboot command.
`config-tools` owns offline input reports, portable materialization/catalogs and
opaque editor selections, direct manifest launch and exact numbered backups.
It validates declarations before materializing files and then execs without
...
bridge; `seele-pi-jj` owns bounded metadata reads. Vicinae snapshots, keybinding
normalization/input, audio revalidation and immutable diff formatting run in
native control endpoints. Native `seele-agent-hook` owns lifecycle publication. Spicetify's one-line accent
adapter requires browser localStorage. Proton's
Python compatibility patch stays inside its upstream authentication package.
...
`how` and `debug` replace the live buffer, while other input executes normally.
A resident Rust wrapper forwards the stderr PTY byte-for-byte and retains at most
64 KiB plus the last sanitized failure in memory. A private same-user socket
carries preexec/postexec boundaries and explicit debug reads; no command output
or failure record is persisted. Both modes share a bounded metadata collector
...
and screen preview gates. Context request tokens reject late callbacks. Send starts `codex exec` through the shared no-tools policy in read-only mode from
...
`modules/hosts/nerv/memory-pressure.nix` owns the low-memory policy that sits on top of that swap configuration. `systemd.oomd.enable` is already true by default in nixpkgs while `enableRootSlice`, `enableSystemSlice`, and `enableUserSlices` all default to false, so the daemon runs with nothing under management; this leaf enables the system slice only. The root slice is left off because monitoring `-.slice` makes system.slice and user.slice the candidates, and the user slices are left off because `uwsm` runs Hyprland as `wayland-wm@Hyprland.service` and everything the compositor launches with `exec` stays in that one session cgroup, so oomd's only worthwhile candidate inside the session would take the compositor, Seele Shell, and every open window at once. Per-application candidates would first require applications started through `uwsm app`. `ManagedOOMSwap` stays unset everywhere, because zram at `vm.swappiness = 100` is designed to fill and swap usage carries no distress signal on this host; the slice uses `ManagedOOMMemoryPressureLimit` at 60% over a 15-second window instead of oomd's 80% over 30 seconds. earlyoom is deliberately absent for the same reason, and because it picks single processes by RSS with no knowledge of which one holds the display up. `nix-daemon.service` carries the whole build load under `max-jobs = 2` and `cores = 6`, so it takes `OOMScoreAdjust = 500` — the kernel scales that by total memory, making builds look about 16 GiB larger to its own OOM killer on this 32 GiB machine — and `MemoryHigh = "16G"`, which throttles into reclaim rather than failing the build the way `MemoryMax` would; `nix-daemon.socket` restarts the daemon on the next connection. `systemd-logind` and `greetd` are marked `ManagedOOMPreference = "avoid"` because they own the way back to a usable screen. The session is discounted through `systemd.services."user@".serviceConfig.OOMScoreAdjust = -250`, set from the system manager because a user manager has no `CAP_SYS_RESOURCE` and would silently drop a negative value. The NixOS oomd module supplies `ManagedOOMMemoryPressure = "kill"` and an `lib.mkDefault` limit for the enabled slice, so the explicit limit here overrides it without conflict.
...
`modules/features/system/foreign-binaries.nix` is the one leaf that concerns software this flake does not package. `modules/features/system/linux.nix` imports its `flake.modules.nixos.foreign-binaries` into the NixOS `linux` profile, so it reaches `nerv` and any later NixOS host rather than being host-scoped. nix-ld installs itself as `environment.ldso`, which answers the `/lib64/ld-linux-x86-64.so.2` a foreign binary was linked against and exports `NIX_LD` and `NIX_LD_LIBRARY_PATH` for it. The nixpkgs module assigns `programs.nix-ld.libraries` from inside its own `config` rather than as the option's default, so the list this leaf adds — expat, fontconfig, freetype, icu, libffi, the libraries a prebuilt command-line tool or vendored language server reaches for — merges with the systemd and Nix libraries upstream already lists instead of shadowing them. That merge is the reason for reading the upstream module before extending the list; a graphical toolkit belongs there only once a binary that needs it exists, because everything listed is realized into the system closure. `programs.appimage` with `binfmt = true` registers both AppImage magic types against `appimage-run`, so an executable AppImage runs from a shell or a file manager without naming the runner. Neither option is a substitute for packaging: `modules/packages/t3code.nix` still extracts its AppImage through `appimageTools`, which is what anything kept on the machine should do, and `comma` remains the answer for a command that nixpkgs does package.

`modules/features/system/disk-health.nix` is the leaf that watches the hardware the rest of this flake assumes is healthy. `modules/features/system/linux.nix` imports its `flake.modules.nixos.disk-health` into the NixOS `linux` profile, so it covers `nerv` and any later NixOS host rather than being host-scoped, and it names no device: every filesystem here is mounted by UUID so a disk can be replaced without editing configuration, and `autodetect` keeps the replacement monitored. `defaults.autodetected` extends upstream's `-a` -- health verdict, error and self-test logs, reallocated and pending sector counters -- with `-s (S/../.././02|L/../../6/03)`, a short self-test nightly at 02:00 and a long one Saturday at 03:00, because polling reports only what the drive already noticed while a self-test exercises regions nothing has read. A drive that does not support self-tests logs a refusal and stays monitored by everything else. The notification path is the part to read the upstream module before changing. A root service cannot reach the session bus Seele Shell's notification server listens on, so smartd broadcasts `net.nuetzlich.SystemNotifications.Notify` on the system bus and `services.systembus-notify` -- which the smartd module enables through `lib.mkDefault` alongside `notifications.systembus-notify.enable` -- forwards it into `graphical-session.target`. Upstream builds smartd's `-M exec` argument from the mail, wall and X11 toggles only; the systembus-notify toggle is not part of that condition, so disabling the other three would leave the notify script unattached and every desktop warning silently gone. `wall.enable = true` is therefore stated rather than left to its default. X11 notifications are explicitly off: `modules/hosts/nerv/xserver.nix` enables `services.xserver` for the German keymap, which is what upstream reads to decide, but the session is Hyprland, so xmessage could only add itself to the system closure. `environment.systemPackages` carries a one-file derivation rather than `pkgs.smartmontools` itself: the package installs `smartctl` into `sbin`, NixOS puts only the profile's `/bin` on `PATH`, and the leaf links whichever directory the package actually ships so the manual command is reachable by name and a changed layout fails the build instead of installing a dangling link. `seele-disk-health-test` is dormant on purpose, in the same spirit as `seele-failure-test`: it sends exactly the two strings smartd sends, so `systemctl start seele-disk-health-test` proves the root-to-session path on the live machine without failing hardware. smartd stopping needs no wiring of its own, because the failure-analysis generator attaches its `OnFailure=` reporter to installed system services. This is early warning, not durability: restoring from a disk that has died is still SIL-28's unimplemented backup work.
...
`modules/hosts/nerv/dns.nix` owns the host's resolver. It enables `services.resolved` through the freeform `settings.Resolve` attribute set, which is the only interface current nixpkgs offers: the flat `dnssec`, `dnsovertls`, `domains`, `fallbackDns`, and `llmnr` options are renamed aliases into it, and `extraConfig` was removed outright. The system servers are Quad9's four addresses, each suffixed `#dns.quad9.net` so strict `DNSOverTLS` validates the certificate against that name and sends it as SNI; `FallbackDNS` is pinned to the same host so systemd's compiled-in Cloudflare and Google list can never answer. `Domains = [ "~." ]` is what keeps the leased resolver out of the path: resolved routes a name to the servers behind the longest matching routing domain and only falls back to every default-route link when nothing matched, so without the root domain ordinary names reach the router and Quad9 in parallel. Longer domains still win, which is why tailscaled's `~ts.net` split DNS and the lease's search and reverse zones are unaffected. `DNSSEC` stays false because Quad9 validates upstream over an already authenticated channel, local validation adds a second validator that fails closed on a revoked trust anchor, and `allow-downgrade` is forgeable by the network; tailscaled's own per-link `DNSSEC=no` is best-effort and only logged on failure, so the tailnet does not depend on it. `LLMNR` is false and `MulticastDNS` is `resolve`. The same leaf sets `networking.networkmanager.dns` to `systemd-resolved` — which resolved's module also asserts, and the enum merges equal definitions — and uses `connectionConfig` numeric enums to give every managed profile `connection.dns-over-tls = 0`, `connection.llmnr = 0`, and `connection.mdns = 1`. Per-link DNS-over-TLS off matters: resolved applies the global mode to unset links, so the router's plaintext resolver would otherwise be queried on port 853 for the very names it is authoritative for. The host registers no `seele.health.providers` entry, because that contract expects an integration that publishes bounded samples from the shell and a registration with no publisher would only ever read as stale.
...
`modules/features/themes/cursor.nix` themes the pointer from the same shared values. It builds the `catppuccin-cursors` output name as `<flavor><Accent>` and the installed theme directory as `catppuccin-<flavor>-<accent>-cursors`, then publishes two modules. `flake.modules.homeManager.cursor`, imported by `modules/profiles/home/linux.nix`, sets `stylix.cursor` rather than `home.pointerCursor`: Stylix applies that option whenever `stylix.enable` is set instead of gating it on a target, it defines `home.pointerCursor` itself, and the active `gtk`, `kde`, and `x11` targets read it to theme GTK, Qt/KDE, and X clients. The same module writes `XCURSOR_THEME` and `XCURSOR_SIZE` into Hyprland's own configuration, because the compositor draws the pointer itself and reads the theme while it parses that configuration, earlier than the session variables Home Manager writes for shells; the clients it spawns, XWayland included, inherit the pair. `flake.modules.nixos.cursor`, imported by `modules/features/system/linux.nix` beside the Catppuccin and Stylix system modules, installs the theme into the system profile, names it in `xdg.icons.fallbackCursorThemes` so the `default` theme resolves to it, and exports the same variables through `environment.sessionVariables`. That is what covers greetd's greeter, which runs as its own user and cannot read `~/.icons`. `asuka` draws the macOS pointer and imports neither module.
...
30-second notification actions for local view or explicit broker analysis. Local
...
keeps its row until its node leaves the graph, so a correctly role-tagged system
ding never earns one and a pause between two tracks does not take a row out from
under the pointer. A roleless ding can appear for as long as its node exists.
...
The audio device list is not just the sink and source nodes. A card whose profile is `Off` has no sink node at all, so enumerating nodes alone hides every output that is merely switched off — onboard analog and HDMI are the usual casualties, and the list collapses to whichever interface happens to be active. the native audio-device collector therefore adds a second kind of entry, built from each `Audio/Device`'s `EnumProfile`, gated on the card having no sink already *and* the profile reporting `available: yes`; that gate is what keeps unplugged jacks and every `Pro Audio` variant out of the list. Those entries carry a `profile` index instead of a node name, and `seele-control audio-device <device> <profile>` switches the card with `wpctl set-profile` and then waits for the node PipeWire creates for it before making that the default — the sink does not exist at the moment of the click, so without the second step the choice only half-applies.
...
`seele.codexBroker`, not consumer requests. Validate the Rust lifecycle suite, socket fixture
and real-Codex loopback fixture before changing broker or Codex integration.
...
output and AI readings are quoted into wells, timestamps and recurrences are a
label column against a pinned value column, snoozing lives in the fold, a repair
that needs agreeing to carries both ways out in a banner where it will run, and
work in flight is a spinner in place. Checks that could not run are one banner
