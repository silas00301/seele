...
model inference, recognition quality and GPU execution remain unverified. The source archive NAR hash was
computed without Nix using a serializer cross-checked against the existing
flake-utils lock hash. Large model hashes come from the pinned Hugging Face LFS
metadata; the vocabulary hash was computed from downloaded bytes.

Nix is unavailable and was not installed, as requested. `nix fmt` could not run;
the existing standalone nixfmt formats all parent-repository Nix files instead.
Flake evaluation/checks, the CUDA package and `nerv` builds remain unrun.
