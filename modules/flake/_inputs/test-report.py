...
HELPER = Path(__file__).with_name("report.py")
spec = importlib.util.spec_from_file_location("inputs_report", HELPER)
...
class InputReportTests(unittest.TestCase):
    ...
    def test_legal_dependency_cycle_and_empty_follows_are_reported(self):
        ...
        self.assertIn("cycle to <root>", report.render(rows))
    ...
    def test_url_redaction(self):
        cases = {
            "https://user:secret@host.example:443/path?token=secret#secret": "https://host.example:443/path",
            "git+ssh://user:secret@[::1]:22/repo?secret#secret": "git+ssh://[::1]:22/repo",
            "git@host.example:org/repo?secret#secret": "host.example:org/repo",
            "user:secret@host.example:org/repo?secret": "host.example:org/repo",
            "https://user:secret@[invalid/path": "[URL omitted]",
        }
        for value, expected in cases.items():
            with self.subTest(value=value):
                self.assertEqual(report.safe_url(value), expected)
    ...
    def test_empty_lock_and_terminal_controls(self):
        ...
        self.assertEqual(report.render([]), "No inputs.")
        self.assertEqual(report.terminal_text("a\t\x1b\n"), "a\\t\\u001b\\n")
    ...
    def test_cli_flags_json_errors_and_file_remains_unchanged(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            run = lambda *args: subprocess.run([sys.executable, str(HELPER), *args], cwd=root, env=env, text=True, capture_output=True)
