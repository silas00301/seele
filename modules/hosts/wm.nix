...
let
  inherit (config.dotfiles) catppuccin username;
  modules = config.flake.modules;
in
{
  flake.darwinConfigurations.wm = withSystem "aarch64-darwin" (
    perSystemArgs:
    let
  ...
  flake.darwinPackages = config.flake.darwinConfigurations.wm.pkgs;
}
