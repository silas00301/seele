...
`nerv` runs Determinate Nix. `modules/features/system/determinate.nix` publishes `flake.modules.nixos.determinate` around the `determinate` input's NixOS module, and `modules/hosts/nerv/determinate.nix` imports it into the host aggregate. That module owns `nix.package` and the daemon, and it redirects the generated `/etc/nix/nix.conf` to `/etc/nix/nix.custom.conf`, so ordinary `nix.settings` remain the way to configure Nix. Keep the `determinate` input free of a nixpkgs `follows`, and leave `asuka` on stock Nix.
